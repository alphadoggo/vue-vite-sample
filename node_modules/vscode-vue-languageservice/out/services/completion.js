...
function register({ modules: { html, emmet, typescript: ts }, sourceFiles, getTsLs, htmlLs, pugLs, getCssLs, jsonLs, documentContext, vueHost, templateTsLs }, getScriptContentVersion) {
        ...
        async function getHtmlResult(sourceFile) {
            ...
            for (const sourceMap of [...sourceFile.getHtmlSourceMaps(), ...sourceFile.getPugSourceMaps()]) {
                ...
                for (const htmlRange of sourceMap.getMappedRanges(position)) {
                    ...
                    let vueItems = htmlResult.items.map(htmlItem => (0, transforms_1.transformCompletionItem)(htmlItem, htmlRange => sourceMap.getSourceRange(htmlRange.start, htmlRange.end)));
            ...
            for (const sourceMap of sourceFile.getCssSourceMaps()) {
                ...
                for (const cssRange of cssRanges) {
                    ...
                    const vueItems = cssResult.items.map(cssItem => {
                        ...
                        const vueItem = (0, transforms_1.transformCompletionItem)(cssItem, cssRange => sourceMap.getSourceRange(cssRange.start, cssRange.end));
                        vueItem.data = data;
                        return vueItem;
                    });
                    result.items = result.items.concat(vueItems);
            ...
            for (const sourceMap of sourceFile.getJsonSourceMaps()) {
                ...
                for (const cssRange of jsonRanges) {
                    ...
                    const vueItems = jsonResult.items.map(jsonItem => {
                        const vueItem = (0, transforms_1.transformCompletionItem)(jsonItem, jsonRange => sourceMap.getSourceRange(jsonRange.start, jsonRange.end));
                        return vueItem;
                    });
                    result.items = result.items.concat(vueItems);
                }
            ...
            if (embededDoc) {
                ...
                if (emmetConfig) {
                    ...
                    if (emmetResult && embededDoc.sourceMap) {
                        return (0, transforms_1.transformCompletionList)(emmetResult, emmetRange => embededDoc.sourceMap.getSourceRange(emmetRange.start, emmetRange.end));
                    }
                    return emmetResult;
                }
            }
    ...
    function useComponentCompletionData(sourceFile) {
        const { sfcTemplateScript, templateScriptData, } = sourceFile.refs;
        ...
        const result = (0, reactivity_1.computed)(() => {
            ...
            if (doc) {
                ...
                const globalBind = (_f = (_e = templateTsLs.__internal__.doCompleteSync(doc.uri, doc.positionAt(doc.getText().indexOf(string_1.SearchTexts.GlobalAttrs)))) === null || _e === void 0 ? void 0 : _e.items) !== null && _f !== void 0 ? _f : [];
