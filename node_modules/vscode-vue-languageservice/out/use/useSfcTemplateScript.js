...
function useSfcTemplateScript(vueUri, vueDoc, template, styles, templateScriptData, styleDocuments, styleSourceMaps, templateData, sfcTemplateCompileResult, sfcStyles, scriptLang, context) {
    let version = 0;
    ...
    const templateCodeGens = (0, reactivity_1.computed)(() => {
        ...
        if (!((_a = sfcTemplateCompileResult.value) === null || _a === void 0 ? void 0 : _a.ast))
            return;
        return templateGen.generate(templateData.value.sourceLang, sfcTemplateCompileResult.value.ast, context.isVue2Mode, [...cssScopedClasses.value.values()].map(map => [...map.keys()]).flat(), templateData.value.htmlToTemplate);
    });
