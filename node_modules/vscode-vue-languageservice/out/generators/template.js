...
function generate(sourceLang, templateAst, isVue2, cssScopedClasses = [], htmlToTemplate) {
    ...
    function visitElementNode(node, parentEl) {
        ...
        const name2 = (0, shared_1.camelize)(node.tag); // helloWorld
        ...
        function writeOptionReferences(node) {
            ...
            function write(option, propName, start, end, checking = false, rename = true) {
                ...
                if (option === 'props') {
                    props.add(propName);
                    props.add((0, shared_1.camelize)(propName));
                }
                else if (option === 'emits') {
                    emits.add(propName);
                    emits.add((0, shared_1.camelize)(propName));
                    props.add((0, shared_1.camelize)('on-' + propName));
                }
        ...
        function writeProps(node, forDirectiveClassOrStyle) {
            ...
            for (const prop of node.props) {
                if (prop.type === 7 /* DIRECTIVE */
                    ...
                    const propName_2 = (0, shared_1.hyphenate)(propName_1) === propName_1 ? (0, shared_1.camelize)(propName_1) : propName_1;
            ...
            function addStartWrap() {
                // start tag
                addTag(node.loc.start.offset + node.loc.source.indexOf(node.tag), node.loc.start.offset + node.loc.source.indexOf(node.tag) + node.tag.length);
                if (!node.isSelfClosing && sourceLang === 'html') {
                    // end tag
                    addTag(node.loc.start.offset + node.loc.source.lastIndexOf(node.tag), node.loc.start.offset + node.loc.source.lastIndexOf(node.tag) + node.tag.length);
                }
                tsCodeGen.addText(`__VLS_componentProps[${var_correctTagName}](`);
                const diagStart = tsCodeGen.getText().length;
                tsCodeGen.addText(`{\n`);
